...
TEST test_build_quit_packet()
{
    ...
    err = trilogy_build_quit_packet(&builder);
    ASSERT_OK(err);

    static const uint8_t expected[] = {0x01, 0x00, 0x00, 0x00, 0x01};
...
int build_quit_packet_test()
{
    RUN_TEST(test_build_quit_packet);
