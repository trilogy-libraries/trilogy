...
int greatest_do_assert_equal_t(const void *expd, const void *got, greatest_type_info *type_info, void *udata);
...
#if (defined(__STDC_VERSION__) && __STDC_VERSION__ >= 19901L) || (defined(_MSC_VER) && _MSC_VER >= 1800)
...
#define GREATEST_FAILURE_ABORT() (GREATEST_FIRST_FAIL() && (greatest_info.suite.failed > 0 || greatest_info.failed > 0))
...
#define GREATEST_ASSERT_EQ_FMT(EXP, GOT, FMT) GREATEST_ASSERT_EQ_FMTm(#EXP " != " #GOT, EXP, GOT, FMT)
#define GREATEST_ASSERT_IN_RANGE(EXP, GOT, TOL) GREATEST_ASSERT_IN_RANGEm(#EXP " != " #GOT " +/- " #TOL, EXP, GOT, TOL)
...
#define GREATEST_ASSERT_STRN_EQ(EXP, GOT, SIZE) GREATEST_ASSERT_STRN_EQm(#EXP " != " #GOT, EXP, GOT, SIZE)
#define GREATEST_ASSERT_MEM_EQ(EXP, GOT, SIZE) GREATEST_ASSERT_MEM_EQm(#EXP " != " #GOT, EXP, GOT, SIZE)
#define GREATEST_ASSERT_ENUM_EQ(EXP, GOT, ENUM_STR) GREATEST_ASSERT_ENUM_EQm(#EXP " != " #GOT, EXP, GOT, ENUM_STR)
...
#define GREATEST_CLOCK_DIFF(C1, C2)                                                                                    \
    GREATEST_FPRINTF(GREATEST_STDOUT, " (%lu ticks, %.3f sec)", (long unsigned int)(C2) - (long unsigned int)(C1),     \
...
#define GREATEST_MAIN_DEFS()                                                                                           \
    ...
    {                                                                                                                  \
        struct greatest_run_info *g = &greatest_info;                                                                  \
        if (GREATEST_IS_VERBOSE()) {                                                                                   \
            GREATEST_FPRINTF(GREATEST_STDOUT, "FAIL %s: %s (%s:%u)", g->name_buf, g->msg ? g->msg : "", g->fail_file,  \
                             g->fail_line);                                                                            \
        } else {                                                                                                       \
    ...
    {                                                                                                                  \
        if (greatest_info.suite.tests_run > 0) {                                                                       \
            GREATEST_FPRINTF(GREATEST_STDOUT, "\n%u test%s - %u passed, %u failed, %u skipped",                        \
                             greatest_info.suite.tests_run, greatest_info.suite.tests_run == 1 ? "" : "s",             \
                             greatest_info.suite.passed, greatest_info.suite.failed, greatest_info.suite.skipped);     \
    ...
    unsigned int greatest_get_verbosity(void) { return greatest_info.verbosity; }                                      \
                                                                                                                       \
    void greatest_set_verbosity(unsigned int verbosity) { greatest_info.verbosity = (unsigned char)verbosity; }        \
                                                                                                                       \
    void greatest_set_flag(greatest_flag_t flag)                                                                       \
    {                                                                                                                  \
